...
bool AVSynchronizer::init(DecoderRequestHeader *requestHeader, JavaVM *g_jvm, jobject obj,float minBufferedDuration, float maxBufferedDuration) {
	...
	//1、创建decoder实例----有可能创建ffmepg软解码，也有可能创建mediacodec硬解码
...
/**
 * 已经绘制到到FBO了，这里要考虑如何上屏
 * 因为绘制到FBO，然后我们需要通过这个FBO绑定的textureID，也就是这个inputTexId进行传递
 * @param inputTexId
 * @param width
 * @param height
 * @param position
...
//		LOGI("Render To TextureQueue texture Position is %.3f ", position);
		...
		passThorughRender->renderToTexture(inputTexId, frameTexture->texId); // 这里inputTexId就是FBO绑定的，frameTexture->texId绘制屏幕的
